...
class QuadrupedGymEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self._env_step_counter % 50 == 0 and np.random.random() > 0.0: #0.8
            ...
            # dvx_cmd = (-0.5 + 1.0 * np.random.random())*0.25
            # dvy_cmd = (-0.25 + 0.5 * np.random.random())*0.25
            # dyaw_rate_cmd = (-1.0 + 2.0 * np.random.random())*0.25
            # self.vxCommand = self.vxCommand + dvx_cmd
            # self.vyCommand = self.vyCommand + dvy_cmd
            # self.yaw_rate_cmd = self.yaw_rate_cmd + dyaw_rate_cmd
            # self.vxCommand = np.clip(self.vxCommand, -1.0, 1.0)
            # self.vyCommand = np.clip(self.vyCommand, -0.5, 0.5)
            # self.yaw_rate_cmd = np.clip(self.yaw_rate_cmd, -2.0, 2.0)
        ...
        if self._env_step_counter % 10 == 0 and np.random.random() > 0.5:
            ...
            # self.x_force = 0.0 # Removes the disturbance in env
            # self.y_force = 0.0
            # self.x_tau = 0.0
            # self.y_tau = 0.0
            # self.z_tau = 0.0
